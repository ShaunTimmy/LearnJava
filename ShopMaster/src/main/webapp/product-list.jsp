...
<section class="hero-section" style="background-color: #70a1ff; color: #fff;">
            ...
            <div class="card">
                <div class="image-container">
                    ...
                    <div class="overlay-buttons">
                        <a href="product?action=details&idproduct=<%=sp.getIdproduct()%>" class="btn btn-outline-info btn-sm">
                        ...
                        <a href="<%=link%>" class="btn btn-outline-success btn-sm ms-1 <%=isDisabled ? "disabled" : ""%>"
                           <%=isDisabled ? "aria-disabled='true' tabindex='-1'" : ""%>><i class="fa-solid fa-comment"></i> Đánh giá</a>
                    </div>
                </div>
                <div class="card-body">
                    <h5 class="card-title"><%=sp.getProName()%></h5>
                    <p class="card-text"><strong>Giá: </strong><%=decimalFormat.format(sp.getPrice())%> VND</p>
                    <div class="d-flex justify-content-between">
                        ...
                        %>
                        <a href="cart?action=add&idproduct=<%= sp.getIdproduct() %>&idtaikhoan=<%= idtaikhoan %>&slg=1" 
                           class="btn btn-outline-dark btn-sm"><i class="fa-solid fa-cart-shopping"></i> Giỏ hàng</a>
                        <a href="wishlist?action=add&idproduct=<%= sp.getIdproduct() %>&idtaikhoan=<%= idtaikhoan %>" 
                           class="btn btn-outline-warning btn-sm ms-1"><i class="fa-solid fa-heart"></i> Yêu thích</a>
                        <% 
                            } else {
                        %>
                        <button class="btn btn-outline-warning btn-sm ms-1" disabled><i class="fa-solid fa-heart"></i></button>
                        <% 
                            }
                        %>
                    </div>
