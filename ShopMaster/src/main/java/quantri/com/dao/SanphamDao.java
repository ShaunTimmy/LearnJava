...
public class SanphamDao {
	...
	private static final String SEARCH_PRODUCT="SELECT * FROM sanpham  WHERE proName LIKE ? LIMIT ? OFFSET ? ";
	private static final String SELECT_PRODUCT_BY_CATEGORY = "SELECT * FROM sanpham WHERE theloai = ? LIMIT ? OFFSET ?";
	private static final String SELECT_COUNT_PRODUCT_BY_CATEGORY=" SELECT COUNT(*) FROM Sanpham WHERE theloai = ?";
	private static final String SELECT_PRODUCT="SELECT * FROM sanpham LIMIT 3";
	// Them
	...
	//Tim kiem ben khạch hang
		public List<Sanpham> CustomersearchProduct(String keyword,int offset, int limit) throws SQLException{
			List<Sanpham> product=new ArrayList<Sanpham>();
			try(Connection conn=DataConnection.getConnection();
					PreparedStatement pst=conn.prepareStatement(SEARCH_PRODUCT)){
				String searchKeyword = "%" + keyword + "%";
